...
const main = async (args: any, hre: HardhatRuntimeEnvironment) => {
  ...
  console.log("Fetching remaining seconds in time window...");
  ...
  try {
    ...
    const remainingTimeSeconds = await zrc20Wrapper.getRemainingTimeWindow();

    // Output result
    if (args.json) {
      console.log(
        JSON.stringify({
          remainingTimeSeconds: remainingTimeSeconds.toString(),
        })
      );
    } else {
      console.log(
        `Remaining Time In Seconds: ${remainingTimeSeconds.toString()}`
      );
    }
  } catch (error: any) {
...
task(
  "getTimeWindow",
  "Fetch the remaining time window in seconds from ZRC20Wrapper"
)
  .addOptionalParam(
    "contract",
